...
export default function Login() {
  ...
  const local: { email: string; firstName: string; lastName: string } = useLocalSearchParams();
  ...
  const onSubmit = async (data: SignupTypes) => {
    ...
    if (user) {
      user.set('username', local.email.toLowerCase());
      user.set('email', local.email.toLowerCase());
      ...
      user.set('user_type', data.userType);
      user.set('vat', data.vat);
      user.set('company_name', data.company_name);
      user.set('terms', data.terms);
      user.set('privacy', data.privacy);
      user.set('share_consent', data.userType === 'agent' ? true : data.share_consent);
      user.set('social_signup', true);

      const customACL = new Parse.ACL();
      customACL.setPublicReadAccess(true);
      customACL.setPublicWriteAccess(true);
      // customACL.setWriteAccess(user, true);
      ...
      await fetch(emailsAddress, {
        method: 'POST',
        body: JSON.stringify({ email: 'account_greetings', id: user?.id }),
      });
      ...
      <KeyboardAvoidingView>
        <ScrollView
          ...
          <View className="mb-14 flex-1 flex-col  justify-center">
            <AppText className="mt-5 text-left font-semibold text-3xl">
