...
export default function LocationInfo({ data, onSubmit, onBack,
  ...
  const onSubmitInternal = async (formData: LocationInfoTypes) => {
    if (!formData.searched) {
      addToast({
        heading: 'Auto Search',
        message: 'Click on Auto Search before continuing.',
        type: 'error',
      });
      return;
    }
  ...
  const currentDistrict = useWatch({ control, name: 'district' });
  const currentArea1 = useWatch({ control, name: 'area_1' });
  const currentArea2 = useWatch({ control, name: 'area_2' });
            ...
            <View style={[]}>
              <PressableView
                onPress={async () => {
                  ...
                  const result = await Parse.Cloud.run('geo-code', {
                    address:
                      address +
                      ', ' +
              ...
              >
                <AppText style={styles.continueBtnText}>Auto Search</AppText>
              </PressableView>
            </View>
            <AppText style={styles.infoText}>
              This enables exact property location pin on the map, if disabled
              we will show a circle with a range of 100m on the property
              location
            </AppText>

            <AppText style={styles.infoText}>
              If Google maps find a partial match, drag the pin in the
              appropriate area to locate your property.
            </AppText>
